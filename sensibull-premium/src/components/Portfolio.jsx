...
const PositionRow = ({ symbol, strike, type, qty, avgPrice, ltp, pnl }) => (
  <tr className="border-b border-[#1F2A44]/30 hover:bg-[#1F2A44]/20 transition-colors">
    ...
    <td className="py-4 text-center font-mono text-xs">{qty}</td>
    ...
    <td className="py-4 text-right pr-4">
      <div className={`flex flex-col items-end ${pnl >= 0 ? 'text-[#00C48C]' : 'text-[#FF4D4F]'}`}>
        ...
        <span className="text-[9px] font-bold">
          {pnl >= 0 ? '+' : ''}{((pnl / (avgPrice * qty * 50)) * 100).toFixed(2)}%
        </span>
      </div>
    </td>
  </tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#131B2F] border border-[#1F2A44] p-6 rounded-2xl flex flex-col justify-between">
           ...
           <div className="flex items-baseline gap-2 mt-2">
             <span className="text-2xl font-bold font-mono">₹4,52,380.00</span>
        ...
        <div className="bg-[#131B2F] border border-[#1F2A44] p-6 rounded-2xl flex flex-col justify-between">
           <span className="text-[10px] text-[#8A92A6] uppercase font-bold tracking-widest">Total P&L</span>
           <div className="flex items-baseline gap-2 mt-2">
             <span className={`text-2xl font-bold font-mono ${totalPnl >= 0 ? 'text-[#00C48C]' : 'text-[#FF4D4F]'}`}>
               {totalPnl >= 0 ? '+' : ''}₹{totalPnl.toLocaleString('en-IN')}
             </span>
             <span className="text-xs text-[#8A92A6]">Realized: ₹0.00</span>
           </div>
        </div>
        ...
        <div className="bg-[#131B2F] border border-[#1F2A44] p-6 rounded-2xl flex flex-col justify-between">
           ...
           <div className="flex flex-col mt-2">
             <span className="text-2xl font-bold font-mono">₹1,47,620</span>
             <div className="w-full h-1.5 bg-[#1F2A44] rounded-full mt-2 overflow-hidden">
                <div className="h-full bg-[#00C48C]/50 w-[32%]" />
             </div>
           </div>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-[#1F2A44] flex items-center justify-between bg-[#131B2F]/50">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
               <button className="px-3 py-1 bg-[#00C48C]/10 text-[#00C48C] text-[10px] font-bold rounded-lg border border-[#00C48C]/20">CURRENT (3)</button>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#0B1426]/50 text-[10px] font-bold text-[#8A92A6] uppercase tracking-widest text-left">
                ...
                <th className="py-3 text-right pr-4">Unrealized P&L</th>
              </tr>
            </thead>
            <tbody>
              {positions.length === 0 ? (
                ...
                positions.map((pos, idx) => <PositionRow key={idx} {...pos} />)
              )}
            </tbody>
          </table>
        </div>
